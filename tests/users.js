...
// api key from https://gorest.co.in/
...
// variable to track user and post
let user_id = 0 
let post_id = 0 
...
describe('Users', () => {
    ...
    it('Create a user with custume user data', (done) => {
        console.log('Create a user with custume user data');
    ...
    it('Create a Post', (done) => {
        ...
        request.post(`users/${user_id}/posts`)
            .set("Authorization", `Bearer ${API_KEY}`)
            .send(post_data)
            .end((err, res) => {
    ...
    it('Get saved posts', (done) => {
        ...
        request.get(`users/${user_id}/posts`)
            .end((err, res) => {
    ...
    it('Create Comment', (done) => {
        ...
        request.post(`posts/${post_id}/comments`)
            .set("Authorization", `Bearer ${API_KEY}`)
            .send(comments_data)
            .end((err, res) => {
                expect(res.body.code).to.eql(201)
